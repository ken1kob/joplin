...
function onNotebookSelectionMayChange() {
	...
	if (perNotebookSortOrderEnabled()) {
		if (next.field != field || next.reverse != reverse) {
			CommandService.instance().execute('notesSortOrderSwitch', next.field, next.reverse);
		}
	}
}
...
function getCurrentField(): string {
	return Setting.value('notes.sortOrder.field');
}

function getCurrentReverse(): boolean {
	return Setting.value('notes.sortOrder.reverse');
}

function perFieldReversalEnabled(): boolean {
	return Setting.value('notes.perFieldReversalEnabled');
}

function perNotebookSortOrderEnabled(): boolean {
	return Setting.value('notes.perNotebookSortOrderEnabled');
}
